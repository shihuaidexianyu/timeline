...
- 当窗口指纹变化时结束旧段、创建新段
- 指纹由 `hwnd + process_id` 组成。窗口标题不再参与指纹计算，避免 IDE 切文件、浏览器切标签等标题变化将一段连续焦点误切成大量碎片段。标题仍作为段的元数据被记录，只是不触发段切换
- 相同前台窗口连续轮询不会重复建段
...
- 锁屏或 idle 时关闭当前可见窗口段；用户回到 active 后重新开始计时。这保证可见窗口总时长 ≤ 活跃总时长，语义自洽
